...
@dataclass
class FloorDecision:
    winner: Optional[str]  # agent_id or None if nobody should speak
    scores: dict
    reason: str
...
class SessionStore:
    ...
    async def create(self, candidate_name: str, role: str, channel: str) -> ConversationState:
        session_id = str(uuid.uuid4())[:8]
        state = ConversationState(
            session_id=session_id, candidate_name=candidate_name, role=role, channel=channel
        )
